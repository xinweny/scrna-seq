...
gene.summary <- data.frame(mean_rpm=means.vehicle,
                           cv=cv.vehicle,
...
for (i in 1:length(d)) {
  ...
  gene.summary <- rbind(gene.summary,
...
keep.genes <- gene.summary %>%
  group_by(gene_short_name) %>%
  summarize(mean=mean(mean_rpm)) %>%
  filter(mean > 0) %>%
  .$gene_short_name %>% unique()

gene.summary <- gene.summary[gene.summary$gene_short_name %in% keep.genes, ]

# Set order of samples
gene.summary$sample <- factor(gene.summary$sample, levels=c("Vehicle_0",
                                                            paste0(product, "_10"),
                                                            paste0(product, "_100"),
...
ggplot(gene.summary, aes(x=sample, y=log10(mean_rpm + 1))) +
  geom_violin() +
...
png(glue("./processed/GSE139944/transcriptional_noise/sciPlex3_CV_{cell.type}_{product}.png"),
    width=3000, height=2000, res=300)
ggplot(gene.summary, aes(x=sample, y=cv)) +
  geom_boxplot() +
  ylim(1, 5) +
  xlab("") +
  ylab("CV") +
